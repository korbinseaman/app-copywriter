...
帮助产品经理为 HarmonyOS 系统应用编写专业文案，覆盖功能命名、Slogan、营销话术、交互提示和版本更新日志。

## 触发条件

当用户请求为 HarmonyOS 系统应用编写文案时触发，包括但不限于：
...
不适用于：非 HarmonyOS 平台的文案需求、纯技术文档写作。
...
**缺省处理：** 用户未指定时，默认风格为「科技感 + 简洁」，产出 3 个版本供选择。
...
1. **HarmonyOS 风格优先**：用词、语气、结构对齐 HarmonyOS 设计规范
2. **参考竞品但不抄袭**：借鉴 Apple / Google 的表达思路，转化为 HarmonyOS 语境
3. **多版本输出**：每次至少提供 3 个版本，标注各自的特点和适用场景
4. **标注依据**：关键用词选择附上规范依据（如"根据 HarmonyOS 术语表，应使用'智慧'而非'智能'"）

### 第四步：质量检查

生成后运行 `scripts/copy-validator.py` 进行基础质量检查（如字数超限、禁用词、格式规范），并将检查结果一并展示给用户。

## 文件结构

```
harmonyos-copywriter/
├── SKILL.md
├── references/
│   ├── naming-conventions.md    # 命名规范
│   ├── tone-guide.md            # 语气调性指南
│   ├── terminology.md           # HarmonyOS 术语表
│   └── case-library.md          # Apple / Google / Huawei 案例库
├── assets/
│   ├── templates/
│   │   ├── slogan-template.md
